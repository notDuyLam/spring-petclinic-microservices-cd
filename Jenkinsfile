        ...
        stage('Check Changed Files') {
            steps {
                script {
                    ...
                    env.CHANGED_MODULES = "spring-petclinic-customers-service,spring-petclinic-vets-service,spring-petclinic-visits-service,spring-petclinic-api-gateway,spring-petclinic-genai-service,spring-petclinic-admin-server,spring-petclinic-discovery-server,spring-petclinic-config-server"                
                }
            }
        }
                ...
                script {
                    ...
                    if (modules.size() > 0) {
                        ...
                        for (module in modules) {
                            ...
                            sh "docker tag springcommunity/${module}:latest ${REGISTRY}/${REPO}/${module}:${imageTag}"
                        }
                    }
                }
            }
                ...
                script {
                    ...
                    if (modules.size() > 0) {
                        withCredentials([usernamePassword(
                            ...
                            for (module in modules) {
                                def imageName = "${REGISTRY}/${REPO}/${module}:${env.IMAGE_TAG}"
